      ...
      <div class="c_first" v-if="stepsActive == 0">
        <el-form class="lianshang-form c_form" :rules="firstRules" ref="firstRuleForm" :model="firstRuleForm">
          ...
          <el-form-item label="请选择商品分类" label-width="180px" prop="classifyId">
            ...
            @active-item-change="onProvincesChange" :options="options1" @change="handleValueName" class="c_cascader"></el-cascader>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async handleNextThird () {
      ...
      //   let arr = {
      //     attachmentNo: item.attachmentNo,
      //     isMain: 2
      //   }
      //   attachmentNo.push(arr)
      // })
      // attachmentNo.map((item, index) => {
      //   if (index + 1 === this.figureIndex) {
      //     item.isMain = 1
      //   }
      // })
      // console.log(this.oldParamData, '000000000this.oldParamData', this.paramData)
      ...
      specialtempList.map(item => {
        ...
        //   let arr = {
        //     paramName: item.params,
        //     paramNo: item.sku.toString(),
        //     vals: items
        //   }
        //   specialtemp.push(arr)
        // })
        let brr = {
      ...
      const { $api, $message } = this
      try {
        let {transactionStatus} = await $api.product.productAddition(params)
    ...
    async pushFileData () {
      ...
      if (temp.length === 0) {
        ...
        try {
          ...
          if (transactionStatus.success) {
            console.log(1)
            this.attachmentNoList = attachmentNos
            console.log(this.stepsActive, 'ppppppppppp')
            this.attrInit()
            if (this.stepsActive++ > 2) return false
          } else {
            $message.error(transactionStatus.replyText)
